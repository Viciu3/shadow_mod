...
class ShadowUltimat(loader.Module):
    ...
    async def client_ready(self):
        try:
            ...
            if not self.db.get(self.__class__.__name__, "prefix", None):
    ...
    async def main_loop(self):
        ...
        while True:
            try:
                if self.config["PeopleEnabled"] and (not self.db.get(self.__class__.__name__, "people_time", None) or (time.time() - self.db.get(self.__class__.__name__, "people_time")) >= 1805):
                    await self._parse_people()
                    self.db.set(self.__class__.__name__, "people_time", int(time.time()))

                if self.config["BonusEnabled"] and (not self.db.get(self.__class__.__name__, "bonus_time", None) or (time.time() - self.db.get(self.__class__.__name__, "bonus_time")) >= 24 * 3600):
                    await self._parse_bonus()
                    self.db.set(self.__class__.__name__, "bonus_time", int(time.time()))

                if self.config["FuelEnabled"] and (not self.db.get(self.__class__.__name__, "fuel_time", None) or (time.time() - self.db.get(self.__class__.__name__, "fuel_time")) >= 3629):
                    await self._parse_fuel()
                    self.db.set(self.__class__.__name__, "fuel_time", int(time.time()))

                if self.config["GreenhouseEnabled"] and (not self.db.get(self.__class__.__name__, "greenhouse_time", None) or (time.time() - self.db.get(self.__class__.__name__, "greenhouse_time")) >= 1212):
                    await self._parse_greenhouse()
                    self.db.set(self.__class__.__name__, "greenhouse_time", int(time.time()))

                if self.config["WastelandEnabled"] and (not self.db.get(self.__class__.__name__, "wasteland_time", None) or (time.time() - self.db.get(self.__class__.__name__, "wasteland_time")) >= 15 * 60):
                    await self._parse_wasteland()
                    self.db.set(self.__class__.__name__, "wasteland_time", int(time.time()))

                if self.config["GardenEnabled"] and (not self.db.get(self.__class__.__name__, "garden_time", None) or (time.time() - self.db.get(self.__class__.__name__, "garden_time")) >= 1212):
                    await self._parse_garden()
                    self.db.set(self.__class__.__name__, "garden_time", int(time.time()))

                if self.config["MineEnabled"] and (not self.db.get(self.__class__.__name__, "mine_time", None) or (time.time() - self.db.get(self.__class__.__name__, "mine_time")) >= self.config["MineCooldown"] * 60):
                    await self._mine()
                ...
                if xp_match and water_match and crop_match:
                    ...
                    if self._db['greenhouse']['enabled'] and self._db['greenhouse']['water'] > 0:
                        ...
                        while self._db['greenhouse']['water'] > 0:
                            ...
                            response = await conv.get_response()
                            if "у тебя не хватает" in response.raw_text.lower():
                                await self.client.send_message(self._Shadow_Ultimat_channel, f"Greenhouse error: Insufficient resources for {resource}")
                                break
                            if "вырастил" in response.raw_text.lower():
                                self._db['greenhouse']['stock'][resource] += 1
                                self._db['greenhouse']['water'] -= 1
                                await self.client.send_message(self._Shadow_Ultimat_channel, f"Grew {resource}, water left: {self._db['greenhouse']['water']}")
                            else:
                                await self.client.send_message(self._Shadow_Ultimat_channel, f"Unexpected response for {command}: {response.raw_text}")
                                break
                    if stock_match:
                        stock_text = stock_match.group(1).strip()
                        for crop in self._db['greenhouse']['stock']:
                            amount = re.search(rf"{crop.capitalize()} - (\d+) шт\.", stock_text)
                            self._db['greenhouse']['stock'][crop] = int(amount.group(1)) if amount else 0
                    self.db.set(self.__class__.__name__, "db", json.dumps(self._db))
                    await self.client.send_message(self._Shadow_Ultimat_channel, "Parsed greenhouse data")
                else:
                    await self.client.send_message(self._Shadow_Ultimat_channel, f"Failed to parse greenhouse data: {text}")
